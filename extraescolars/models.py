...
class Extraescolar(models.Model):
    ...
    descripcio_curta = models.CharField(
        'Descripció curta', blank=True, max_length=80,
    ...
    descripcio = models.TextField('Descripció', blank=True,
                                  help_text=_text_md)
    inscripcio_des_de = models.DateTimeField(
        'Inscripció des de', blank=True, null=True, help_text=(
            "Des de quan es permet la inscripció. No es faràn inscripcions "
            "abans d'aquest moment."))
    inscripcio_fins_a = models.DateTimeField(
        'Inscripció fins a', blank=True, null=True, help_text=(
            "Fins a quan es permet la inscripció. No es faràn inscripcions "
            "despés d'aquest moment."))
    ...
    cursos = models.ManyToManyField(
        Curs, blank=True, help_text='Els cursos que es poden inscriure')
...
class Inscripcio(models.Model):
    ...
    confirmat = models.BooleanField(default=False, help_text=(
        "Si s'ha confirmat (ex. si s'ha enviat la inscripció per correu o si "
        "s'ha lliurat a l'AMPA)."))
    pagat = models.BooleanField(default=False, help_text=(
        "Si està pagat. Si el preu de l'activitat és 0 (gratuïta), es marcarà "
        "automàticament al guardar."))
