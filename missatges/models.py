...
class GrupDeMissatgeria(models.Model):
    ...
    motius = models.TextField(
        blank=True,
        help_text=('Motius per enviar missatges a aquest grup. Apareixeràn a '
...
class Missatge(models.Model):
    ...
    estat = models.CharField(
        max_length=8,
        blank=True,
        choices=[('CLOSED', 'Tancat'), ('REOPENED', 'Reobert')])
    ...
    def calcular_destinataris(self, vist=False):
        ...
        if self.per not in self.destinataris.all():
            EstatMissatge.objects.create(
                destinatari=self.per, missatge=self, vist=True)
        for user in self.conversacio.a.usuaris.all():  # pylint: disable=no-member
            if user not in self.destinataris.all():
                EstatMissatge.objects.create(
                    destinatari=user, missatge=self, vist=vist)
        self.save()
    ...
    def notificar(self, request):
        context = {
            ...
            self.conversacio,
            'conversacio_url':
            request.build_absolute_uri(self.conversacio.get_absolute_url()),  # pylint: disable=no-member
            'missatge':
            self.contingut
        }
        ...
        bcc = [
            d.email
            for d in self.destinataris.filter(
                ~Q(email=''),
                email__isnull=False,
            )
...
class Conversacio(models.Model):
    ...
    de = models.ForeignKey(  # pylint: disable=invalid-name
        User,
        on_delete=models.SET_NULL,
        blank=False,
        null=True)
    a = models.ForeignKey(  # pylint: disable=invalid-name
        GrupDeMissatgeria,
        on_delete=models.SET_NULL,
        blank=False,
        null=True)
    ...
    def can_access(self, usuari):
        ...
        accedir
        """
        return (usuari == self.de) or (
            self.a.usuaris.filter(username=usuari.username).exists())
    ...
    def marcar_com_a_llegits(self, usuari):
        ...
        for msg in Missatge.objects.filter(conversacio=self):
            EstatMissatge.objects.update_or_create(
                destinatari=usuari, missatge=msg, defaults={
                    'vist': True
                })
